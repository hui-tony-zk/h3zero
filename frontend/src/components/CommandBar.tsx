import { ArrowRight, ChevronDown, ChevronUp, ExternalLink, LoaderCircle, Zap } from "lucide-react";
import { AnimatePresence, motion } from "framer-motion";
import { useState } from "react";
import { ratiosMatch } from "../lib/assets";
import { hasReferencePromptContent } from "../lib/promptDefaults";
import type { AspectId, BaseDraft, ComposerDraft, GenerationCount, GenerationMode, H3Specs, MediaAsset } from "../types";
...
import { prepareFrame, prepareReferences } from "./composer/validation";
...
export function CommandBar({ draft, specs, open, launching, onOpenChange, onModeChange, onUpdate, onSetFrame, onAddReferences, onRemoveReference, onLaunch }: {
  draft: ComposerDraft;
  specs: H3Specs | null;
  open: boolean;
  launching: boolean;
  ...
  return <>
    <motion.section initial={false} animate={{ height: open ? "90dvh" : 60 }} transition={{ duration: .22, ease: [0.16, 1, 0.3, 1] }} className="h3-bottom-sheet fixed inset-x-0 bottom-0 z-50 mx-auto flex max-h-[90dvh] max-w-[760px] flex-col overflow-hidden rounded-t-[20px] border border-b-0 border-reelo-border bg-[#181818]/98 shadow-[0_-18px_70px_rgba(0,0,0,.62)] backdrop-blur-xl sm:inset-x-6">
      ...
      <motion.div initial={false} animate={{ opacity: 1 }} className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain pb-[env(safe-area-inset-bottom)]">
          <AnimatePresence mode="wait" initial={false}>{draft.mode === "frames" ? <FramesInput draft={draft} accept={frameAccept} onFiles={(role, file) => void attachFrame(role, file)} onView={setViewer} onRemove={(role) => void onSetFrame(role, null)} /> : <ReferencesInput draft={draft} accept={referenceAccept} onFiles={(files) => void attachReferences(files)} onView={setViewer} onRemove={onRemoveReference} />}</AnimatePresence>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-white/7 px-3 py-3 sm:px-4">
            ...
            <button type="button" aria-pressed={draft.turbo} onClick={() => onUpdate({ turbo: !draft.turbo })} title={draft.turbo ? "Turbo sampling on: 8 steps" : "Turbo sampling off: base 20 steps"} className={`inline-flex h-8 items-center gap-1.5 rounded-full border px-2.5 text-[10px] font-bold transition ${draft.turbo ? "border-reelo-accent/45 bg-reelo-accent/12 text-reelo-accent" : "border-white/10 bg-white/[.03] text-reelo-dim hover:border-white/18 hover:text-reelo-text"}`}><Zap size={11} fill={draft.turbo ? "currentColor" : "none"} />Turbo</button>
    ...
    {viewer && <MediaViewer asset={viewer} onClose={() => setViewer(null)} />}
