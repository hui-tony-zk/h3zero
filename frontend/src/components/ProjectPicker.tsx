...
export function ProjectPicker({ job, projects, onCreate, onAdd, onClose }: {
  job: Job;
      ...
      >
        <header className="flex items-start justify-between gap-5 border-b border-white/8 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[11px] leading-4 text-white/45">Projects use the video cached on this device. Favorites and cloud sync stay independent.</p>
          </div>
        ...
        <div className="max-h-[52dvh] overflow-y-auto p-2">
          <button type="button" onClick={createAndAdd} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-white/82 hover:bg-white/6">
          ...
          {projects.map((project) => {
            ...
            return (
              <button key={project.id} type="button" disabled={alreadyAdded} onClick={() => onAdd(project.id)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-white/82 hover:bg-white/6 disabled:opacity-40">
                <span className="flex size-9 items-center justify-center rounded-full border border-white/8 bg-white/[.035] text-white/55"><FolderPlus size={14} /></span>
