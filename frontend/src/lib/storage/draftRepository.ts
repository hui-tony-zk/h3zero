...
import type { AspectId, DraftCollection, FramesDraft, GenerationCount, GenerationMode, MediaAsset, PromptDocument, ReferencesDraft, ResolutionId, SamplingProfileId, SeedChoice, SparseAttentionBudget } from "../../types";
...
export function emptyFramesDraft(): FramesDraft {
  return { mode: "frames", prompt: "", promptDocument: emptyPromptDocument(), duration: 5, aspect: "16:9", generationCount: 2, samplingProfile: "turbo_4", seed: "random", resolution: "480p", turbo: true, sparseAttention: false, sparseAttentionBudget: 0.3, firstFrame: null, lastFrame: null };
}

export function emptyReferencesDraft(): ReferencesDraft {
  return { mode: "references", prompt: "", promptDocument: emptyPromptDocument(), duration: 5, aspect: "16:9", generationCount: 2, samplingProfile: "turbo_4", seed: "random", resolution: "480p", turbo: true, sparseAttention: false, sparseAttentionBudget: 0.3, references: [] };
}
...
function sparseAttentionBudget(value?: number): SparseAttentionBudget {
  return value === 0.1 || value === 0.15 || value === 0.3 ? value : 0.3;
}
...
async function restoreFrames(stored?: StoredDraft, useNewDefault = false): Promise<FramesDraft> {
  ...
  return { mode: "frames", prompt: "", promptDocument: emptyPromptDocument(), duration: stored.duration ?? 5, aspect: aspect(stored.aspect), generationCount: generationCount(stored.generationCount, useNewDefault), samplingProfile: profile, seed: "random", resolution: "480p", turbo: isTurboProfile(profile), sparseAttention: stored.sparseAttention === true, sparseAttentionBudget: sparseAttentionBudget(stored.sparseAttentionBudget), loras: stored.loras ?? {}, firstFrame, lastFrame };
}
...
async function restoreReferences(stored?: StoredDraft, useNewDefault = false): Promise<ReferencesDraft> {
  ...
  const profile = samplingProfile(stored);
  return { mode: "references", prompt: "", promptDocument: emptyPromptDocument(), duration: stored.duration ?? 5, aspect: aspect(stored.aspect), generationCount: generationCount(stored.generationCount, useNewDefault), samplingProfile: profile, seed: "random", resolution: "480p", turbo: isTurboProfile(profile), sparseAttention: stored.sparseAttention === true, sparseAttentionBudget: sparseAttentionBudget(stored.sparseAttentionBudget), loras: stored.loras ?? {}, references };
}
...
function base(draft: FramesDraft | ReferencesDraft): StoredDraft {
  return { duration: draft.duration, aspect: draft.aspect, generationCount: draft.generationCount, samplingProfile: draft.samplingProfile, turbo: draft.turbo, sparseAttention: draft.sparseAttention === true, sparseAttentionBudget: draft.sparseAttentionBudget ?? 0.3, loras: draft.loras ?? {} };
}
